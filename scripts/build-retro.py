...
script=(root/'src/retro.js').read_text(encoding='utf-8').replace('__GYM_ANIMATION__',(root/'src/gym-animation.js').read_text(encoding='utf-8')).replace('__EXPANSION__',(root/'src/expansion.js').read_text(encoding='utf-8')).replace('__SPACE_SCRUB__',(root/'src/space-scrub.js').read_text(encoding='utf-8')).replace('__SHIP_LIDS__',(root/'src/ship-lids.js').read_text(encoding='utf-8')).replace('__SIGNATURE__',(root/'src/signature-rounds.js').read_text(encoding='utf-8')).replace('__ZEUS__',(root/'src/zeus.js').read_text(encoding='utf-8')).replace('__METER_FX__',(root/'src/meter-fx.js').read_text(encoding='utf-8')).replace('__ASSETS__',json.dumps(assets,separators=(',',':'))).replace('__RECTS__',(root/'assets/retro/rects.json').read_text())
