...
def effects(extension):
    return json.dumps({name: base64.b64encode((root / ('assets/effect-'+name+'.'+extension)).read_bytes()).decode('ascii') for name in ['steam','water','tile1','tile2','tile3','tile4','splash','holder','paper','gym-rattle','gym-blast','gym-lower','gym-footsteps','gym-barbell','gym-treadmill','gym-bounce','gym-sign','tally','royal-fanfare','royal-tray','royal-gust','royal-chandelier','royal-guards','royal-armour']}, separators=(',', ':'))
if args.fragment_output:
    ...
    preview = preview.replace('__LEVEL2_BASE64__', base64.b64encode((root / 'assets/final-set-preview.opus').read_bytes()).decode('ascii'))
    preview = preview.replace('__EFFECTS_JSON__', effects('opus'))
...
fragment = fragment.replace('__LEVEL2_BASE64__', base64.b64encode((root / 'assets/final-set-level-two.mp3').read_bytes()).decode('ascii'))
fragment = fragment.replace('__EFFECTS_JSON__', effects('mp3'))
